...
def generate_seed():
    ...
    with open(SEED_PATH, "w") as f:
        ...
        cursor.execute(
            "SELECT x, y, local_x, local_y, map_id, tile_image_id, collision_type FROM tiles"
        )
        tiles = cursor.fetchall()

        if tiles:
            f.write(
                "INSERT INTO phaser_tiles (x, y, local_x, local_y, map_id, tile_image_id, collision_type) VALUES\n"
            )
            batch_size = 1000
        ...
        cursor.execute(
            "SELECT x, y, map_id, object_type, sprite_name, name, item_id, action_type, action_direction, local_x, local_y, movement_type FROM objects"
        )
        objects = cursor.fetchall()

        if objects:
            f.write(
                "INSERT INTO phaser_objects (x, y, map_id, object_type, sprite_name, name, item_id, action_type, action_direction, local_x, local_y, movement_type) VALUES\n"
            )
            values = []
            for o in objects:
                ...
                values.append(
                    f"({x}, {y}, {map_id}, {obj_type}, {sprite}, {name}, {item_id}, {action_type}, {action_dir}, {local_x}, {local_y}, {move_type})"
                )
        ...
        cursor.execute(
            "SELECT source_map_id, x, y, destination_map_id, destination_map, destination_x, destination_y FROM warps"
        )
        warps = cursor.fetchall()

        if warps:
            f.write(
                "INSERT INTO phaser_warps (source_map_id, x, y, destination_map_id, destination_map, destination_x, destination_y) VALUES\n"
            )
            values = []
            for w in warps:
                ...
                values.append(
                    f"({source_id}, {x}, {y}, {dest_id}, {dest_map}, {dest_x}, {dest_y})"
                )
