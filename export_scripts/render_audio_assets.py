...
def encode_audio(source_wav, destination, codec, metadata):
...
def encode_outputs(wav_path, out_dir, asset):
...
def render_gbs_asset(
    gbs_path, asset, out_dir, seconds, fade, sample_rate, *, strict_warnings=False
):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        metadata = encode_outputs(wav_path, out_dir, asset)
...
def write_render_manifest(stage_dir, source_manifest, artifacts, sample_rate):
    path = Path(stage_dir) / RENDER_MANIFEST_NAME
    previous = {"artifacts": []}
    if path.exists():
        previous = json.loads(path.read_text(encoding="utf-8"))
    source_hash = sha256_file(source_manifest)
    if previous.get("sourceManifestSha256") != source_hash:
...
def validate_render_bundle(bundle_dir, source_manifest, *, require_complete=False):
    ...
    if render_manifest.get("schemaVersion") != 1:
...
def main():
    ...
    if not args.manifest.exists():
        ...
        return 1
    if args.sample_rate <= 0 or args.music_seconds <= 0 or args.effect_seconds <= 0:
    ...
    try:
        ...
        write_render_manifest(stage_dir, args.manifest, artifacts, args.sample_rate)
        complete_selection = (
            kinds == {"music", "sfx", "base-cries", "cries", "moves"}
