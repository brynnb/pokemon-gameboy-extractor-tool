...
Pipeline order matters! Key dependencies:
  ...
  5. export_objects.py    - Extracts NPCs, items, signs
  6. update_object_coordinates.py - Applies overworld offsets to object positions
  7. generate_mysql_seed.py - Generates a SQL seed file (optional, for non-Go imports)

After the Python scripts, this also:
  8. Copies pokemon.db to the CaptureQuest public folder
  9. Runs the Go import-phaser tool to sync SQLite → MySQL
"""
import subprocess
import os
import sys
